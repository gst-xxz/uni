<template>
  <view :class="customClass" :style="rootStyle">
    <!--插槽需要脱离父容器文档流，防止父容器固宽固高，进而导致插槽大小被被父容器限制-->
    <view :id="resizeId" :class="cn('absolute min-w-[1px] min-h-[1px]', customContainerClass)">
      <!--被监听的插槽-->
      <slot />
      <!--监听插槽变大-->
      <scroll-view class="absolute top-0 left-0 bottom-0 right-0 z-[-9999] overflow-hidden visibility-[hidden]"
        :scroll-y="true" :scroll-top="expandScrollTop" :scroll-x="true" :scroll-left="expandScrollLeft"
        @scroll="onScrollHandler">
        <view class="transition-[0s] animation-[none]" style="height: 100000px; width: 100000px">
        </view>
      </scroll-view>
      <!--监听插槽变小-->
      <scroll-view class="absolute top-0 left-0 bottom-0 right-0 z-[-9999] overflow-hidden visibility-[hidden]"
        :scroll-y="true" :scroll-top="shrinkScrollTop" :scroll-x="true" :scroll-left="shrinkScrollLeft"
        @scroll="onScrollHandler">
        <view class="transition-[0s] animation-[none]" style="height: 250%; width: 250%"></view>
      </scroll-view>
    </view>
  </view>
</template>
