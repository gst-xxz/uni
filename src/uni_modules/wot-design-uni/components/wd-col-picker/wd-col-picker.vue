<template>
  <div :class="`wd-col-picker ${cell.border.value ? 'is-border' : ''} ${customClass}`" :style="customStyle">
    <div class="wd-col-picker__field" @click="showPicker">
      ...
      <div v-else :class="`wd-col-picker__cell ${disabled && 'is-disabled'} ${readonly && 'is-readonly'} ${alignRight && 'is-align-right'} ${error && 'is-error'
        }  ${size && 'is-' + size}`">
      ...
      <div class="wd-col-picker__selected">
        <scroll-view :scroll-x="true" scroll-with-animation :scroll-left="scrollLeft">
          <div class="wd-col-picker__selected-container">
            <div v-for="(_, colIndex) in selectList" :key="colIndex"
            ...
            <div class="wd-col-picker__selected-line" :style="state.lineStyle">
            </div>
          </div>
        </scroll-view>
      </div>
      <div class="wd-col-picker__list-container">
        <div v-for="(col, colIndex) in selectList" :key="colIndex" class="wd-col-picker__list"
          :style="colIndex === currentCol ? 'display: block;' : 'display: none;'">
          <div v-for="(item, index) in col" :key="index" :class="`wd-col-picker__list-item ${pickerColSelected[colIndex] && item[valueKey] === pickerColSelected[colIndex] && 'is-selected'} ${item.disabled && 'is-disabled'
