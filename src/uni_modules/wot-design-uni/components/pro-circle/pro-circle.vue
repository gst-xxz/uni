<template>
  <view :class="cn('relative inline-block text-center', customClass)" :style="customStyle">
    ...
    <view v-if="!text" class="absolute z-[1] top-[50%] left-[0] w-[100%] -translate-y-[50%] text-slate-600">
      <!-- 自定义提示内容 -->
      <slot></slot>
    </view>

    <text v-else class="absolute z-[1] top-[50%] left-[0] w-[100%] -translate-y-[50%] text-slate-600">
      {{ text }}
    </text>
  </view>
</template>
