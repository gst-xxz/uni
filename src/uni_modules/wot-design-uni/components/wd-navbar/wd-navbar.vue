  ...
  <view :style="{ height: addUnit(height) }">
    <view :class="`wd-navbar ${customClass} ${fixed ? 'is-fixed' : ''} ${bordered ? 'is-border' : ''}`"
      ...
      <view class="wd-navbar__content">
        ...
        <view :class="`wd-navbar__left ${leftDisabled ? 'is-disabled' : ''}`"
          :hover-class="leftDisabled || $slots.left ? '' : 'wd-navbar__left--hover'" hover-stay-time="70"
          @click="handleClickLeft" v-else-if="!$slots.left">
          <pro-icon v-if="leftArrow" name="arrow-left" custom-class="wd-navbar__arrow" />
        ...
        <view :class="`wd-navbar__right ${rightDisabled ? 'is-disabled' : ''}`" @click="handleClickRight"
          v-if="$slots.right || rightText" :hover-class="rightDisabled ? '' : 'wd-navbar__right--hover'"
          hover-stay-time="70">
          <slot name="right" />
