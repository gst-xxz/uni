<template>
  <page-wraper>
    <demo-block title="基础用法">
      <pro-sort-button v-model="value1" title="价格" @change="handleChange1" />
    </demo-block>

    <demo-block title="设置 allow-reset 允许重置按钮">
      <pro-sort-button v-model="value2" title="价格" allow-reset @change="handleChange2" />
    </demo-block>

    <demo-block title="设置 desc-first 优先切换为降序">
      <pro-sort-button v-model="value3" title="价格" desc-first @change="handleChange3" />
    </demo-block>

    <demo-block title="不展示下划线（当只有一个排序按钮时，应取消展示下划线）">
      <pro-sort-button v-model="value4" title="价格" :line="false" @change="handleChange4" />
    </demo-block>
  </page-wraper>
</template>
